...
public class GoToImplementingBehaviorMethod_Action extends BaseAction {
  ...
  public void doExecute(@NotNull final AnActionEvent event, final Map<String, Object> _params) {
    ...
    GoToHelper.showPopupAndSearchNodeInBackground(event.getData(MPSCommonDataKeys.NODE), event.getData(MPSCommonDataKeys.MPS_PROJECT), GoToImplementingBehaviorMethod_Action.this.calcTitle(event.getData(MPSCommonDataKeys.MPS_PROJECT), event.getData(MPSCommonDataKeys.NODE), event), FindUtils.getFinder("jetbrains.mps.lang.behavior.findUsages.ImplementingMethods_Finder"), GoToContextMenuHelper.getRelativePoint(selectedCell, inputEvent));
  }
  private String calcTitle(final MPSProject mpsProject, final SNode node, final AnActionEvent event) {
    return new ModelAccessHelper(mpsProject.getRepository()).runReadAction(new Computable<String>() {
      public String compute() {
        return "Choose overriding method of " + SPropertyOperations.getString(node, MetaAdapterFactory.getProperty(0xceab519525ea4f22L, 0x9b92103b95ca8c0cL, 0x110396eaaa4L, 0x110396ec041L, "name")) + "() to navigate to";
      }
    });
  }
}
