...
public class LanguageConceptsUsagesFinder extends BaseFinder {
  ...
  public SearchResults find(SearchQuery query, @NotNull ProgressMonitor monitor) {
    ...
    try {
      ...
      for (SearchResult<?> sModelSearchResult : modelResults.getSearchResults()) {
        models.add((SModel) sModelSearchResult.getObject());
      }
      SearchScope scope = new ModelsScope(models.toArray(new SModel[0]));
      ...
      return new SearchResults(roots, results.getSearchResults());
    } finally {
      monitor.done();
    }
  }
