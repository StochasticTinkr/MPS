...
public class ShowSuppressedErrors_Action extends BaseAction {
  ...
  public void doExecute(@NotNull final AnActionEvent event, final Map<String, Object> _params) {
    final Wrappers._T<List<CustomizedNavigatable>> navigatables = new Wrappers._T<List<CustomizedNavigatable>>();
    ...
    ((MPSProject) MapSequence.fromMap(_params).get("mpsProject")).getRepository().getModelAccess().runReadAction(new Runnable() {
      public void run() {
        List<SNode> suppresses = AttributeOperations.getAttributeList(((SNode) MapSequence.fromMap(_params).get("selectedNode")), new IAttributeDescriptor.NodeAttribute(MetaAdapterFactory.getConcept(0xceab519525ea4f22L, 0x9b92103b95ca8c0cL, 0x3a98b0957fe8e5d2L, "jetbrains.mps.lang.core.structure.SuppressErrorsAnnotation")));
        navigatables.value = ListSequence.fromList(suppresses).select(new ISelector<SNode, CustomizedNavigatable>() {
          public CustomizedNavigatable select(SNode it) {
            return new CustomizedNavigatable(new NodeNavigatable(((MPSProject) MapSequence.fromMap(_params).get("mpsProject")), SNodeOperations.getPointer(it)), SPropertyOperations.getString(it, MetaAdapterFactory.getProperty(0xceab519525ea4f22L, 0x9b92103b95ca8c0cL, 0x3a98b0957fe8e5d2L, 0x21a1b53c6f2a72edL, "whichError")), "Suppressed", MPSIcons.Actions.SuppressedError);
          }
        }).toListSequence();
    ...
    GoToContextMenuUtil.showMenu(((MPSProject) MapSequence.fromMap(_params).get("mpsProject")), title.value, navigatables.value, new CustomizedNavigatableRenderer(), relativePoint, false);
  }
}
