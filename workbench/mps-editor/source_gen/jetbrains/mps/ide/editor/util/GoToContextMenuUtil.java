...
public class GoToContextMenuUtil {
  public static <T extends Navigatable> void showMenu(Project project, String title, List<T> entries, final BaseRenderer<T> renderer, RelativePoint point, boolean skipSingle) {
    if (ListSequence.fromList(entries).isEmpty()) {
      return;
    }
    openTargets(point, entries, title, renderer, skipSingle);
  }
  public static void showNodeMenu(Project project, String title, List<SNodeReference> nodes, final BaseRenderer renderer, RelativePoint point) {
    ...
    openTargets(point, navigatables, title, renderer, true);
  }
  ...
  private static <T extends Navigatable> void openTargets(RelativePoint p, List<T> targets, String title, ListCellRenderer<T> listRenderer, boolean skipSingle) {
      ...
      new PopupChooserBuilder(list).setTitle(title).setMovable(true).setItemChoosenCallback(new Runnable() {
        ...
        public void run() {
          ...
          for (Object element : selectedElements) {
            Navigatable selected = (Navigatable) element;
