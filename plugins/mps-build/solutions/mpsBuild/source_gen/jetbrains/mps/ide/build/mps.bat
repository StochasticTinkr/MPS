...
SET BITS=64
SET USER_JDK64_FILE=%USERPROFILE%\.MPS2019.1\config\mps%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%USERPROFILE%\.MPS2019.1\config\mps%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%USERPROFILE%\.MPS2019.1\config\mps%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
::set ADDITIONAL_JVM_ARGS=-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5071
:: 64 bit
::set ADDITIONAL_JVM_ARGS=-XX:ReservedCodeCacheSize=240m
::set ADDITIONAL_JVM_ARGS=-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5071

SET IDEA_PATHS_SELECTOR=MPS2019.1
