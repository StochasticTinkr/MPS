...
public class ReferenceableConceptsChecker extends SpecificChecker {
  public static final CheckerCategory ILLEGAL_REFERENCES = new CheckerCategory(KindLevel.MANUAL, "illegal references");
  ...
  private void checkNode(List<NodeReportItem> results, SNode node, SNode refNode, boolean isAncestor, SNode anchor) {
    ...
    if (cncpt instanceof SInterfaceConcept) {
      ListSequence.fromList(results).addElement(new NodeReportItemBase(MessageStatus.ERROR, SNodeOperations.getPointer(node), "Interface instance found! " + node.toString()) {
        @Override
        public ItemKind getIssueKind() {
          return ILLEGAL_REFERENCES.deriveItemKind("interface instance");
        }
      });
      return;
    }
    if (cncpt.isAbstract()) {
      ListSequence.fromList(results).addElement(new NodeReportItemBase(MessageStatus.ERROR, SNodeOperations.getPointer(node), "Abstract concept instance found! " + node.toString()) {
        @Override
        public ItemKind getIssueKind() {
          return ILLEGAL_REFERENCES.deriveItemKind("abstract concept instance");
        }
      });
      return;
    }
