...
public class JUnitExecutor implements Executor {
  ...
  public JUnitExecutor(MPSProject project, com.intellij.execution.Executor executor, JUnitSettings_Configuration jUnitSettings, IDebuggerSettings debuggerSettings, JavaRunParameters_Configuration javaRunParameters, List<ITestNodeWrapper> testNodes) {
    myProject = project;
    myExecutor = executor;
  ...
  public ProcessHandler execute() throws ExecutionException {
    ...
    ProcessHandler commandProcess = new JUnit_Command().setProject_Project(myProject).setDebuggerSettings_String(myDebuggerSettings.getCommandLine(true)).createProcess(myTestNodes, this.prepareJavaParamsForTests(dirLock, dirCachesPath));
  ...
  private void showWarning() {
    TestsUIUtil.notifyByBalloon(myProject.getProject(), myExecutor.getId(), MessageType.WARNING, "Cannot reuse caches, because the chosen directory is locked by another run.\nThe option will be turned off.");
  }
}
